...
StarterGui:SetCore("SendNotification", {
    Title = "WARNING";
    Text = "This library is a fork of the original project with custom modifications and is fully supported";
	...
	Themes = {
		...
		Forest = {
			["Color Hub 1"] = ColorSequence.new({
				...
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(20, 40, 25)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(10, 25, 15))
			}),
			["Color Hub 2"] = Color3.fromRGB(15, 35, 20),
			["Color Stroke"] = Color3.fromRGB(30, 60, 40),
			["Color Theme"] = Color3.fromRGB(0, 150, 80),
			["Color Text"] = Color3.fromRGB(150, 255, 200),
			["Color Dark Text"] = Color3.fromRGB(100, 200, 150)
		},
		Space = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(5, 5, 15)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(10, 10, 25)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(5, 5, 15))
			}),
			["Color Hub 2"] = Color3.fromRGB(8, 8, 20),
			["Color Stroke"] = Color3.fromRGB(60, 0, 120),
			["Color Theme"] = Color3.fromRGB(120, 0, 255),
			["Color Text"] = Color3.fromRGB(200, 150, 255),
			["Color Dark Text"] = Color3.fromRGB(150, 100, 200)
		},
		Peach = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(35, 20, 15)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(50, 30, 25)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(35, 20, 15))
			}),
			["Color Hub 2"] = Color3.fromRGB(45, 25, 20),
			["Color Stroke"] = Color3.fromRGB(80, 50, 40),
			["Color Theme"] = Color3.fromRGB(255, 180, 140),
			["Color Text"] = Color3.fromRGB(255, 210, 190),
			["Color Dark Text"] = Color3.fromRGB(230, 180, 160)
		},
		Steel = {
			["Color Hub 1"] = ColorSequence.new({
			...
			["Color Stroke"] = Color3.fromRGB(70, 70, 75),
			["Color Theme"] = Color3.fromRGB(150, 150, 160),
			["Color Text"] = Color3.fromRGB(220, 220, 230),
			["Color Dark Text"] = Color3.fromRGB(180, 180, 190)
		},
		Lava = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(40, 10, 5)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(60, 15, 10)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(40, 10, 5))
			}),
			["Color Hub 2"] = Color3.fromRGB(50, 15, 10),
			["Color Stroke"] = Color3.fromRGB(80, 30, 20),
			["Color Theme"] = Color3.fromRGB(255, 60, 0),
			["Color Text"] = Color3.fromRGB(255, 150, 100),
			["Color Dark Text"] = Color3.fromRGB(200, 100, 80)
		},
		Ice = {
			["Color Hub 1"] = ColorSequence.new({
				ColorSequenceKeypoint.new(0.00, Color3.fromRGB(15, 25, 35)),
				ColorSequenceKeypoint.new(0.50, Color3.fromRGB(25, 40, 55)),
				ColorSequenceKeypoint.new(1.00, Color3.fromRGB(15, 25, 35))
			}),
			["Color Hub 2"] = Color3.fromRGB(20, 35, 50),
			["Color Stroke"] = Color3.fromRGB(50, 100, 130),
			["Color Theme"] = Color3.fromRGB(100, 200, 255),
...
local Settings = Veloalib.Settings
local Flags = Veloalib.Flags
local Toggles = {}
...
local function SaveCfg(filename)
    local data = {}
    for flag, obj in pairs(Veloalib.Flags) do
        if obj.Save then
            data[flag] = obj.Value
        end
    end
...
local SetProps, SetChildren, InsertTheme, Create do
	InsertTheme = function(Instance, Type)
		table.insert(Veloalib.Instances, {
			Instance = Instance,
			Type = Type
		})
		return Instance
	...
	local function Save(file)
		if readfile and isfile and isfile(file) then
			...
			if type(decode) == "table" then
				if rawget(decode, "UISize") then Veloalib.Save["UISize"] = decode["UISize"] end
				if rawget(decode, "TabSize") then Veloalib.Save["TabSize"] = decode["TabSize"] end
				if rawget(decode, "Theme") and VerifyTheme(decode["Theme"]) then Veloalib.Save["Theme"] = decode["Theme"] end
			end
		end
	end
...
local Connections, Connection = {}, Veloalib.Connection do
...
Veloalib.Save = {
  UISize  = saved.UISize  or Veloalib.Save.UISize,
  TabSize = saved.TabSize or Veloalib.Save.TabSize,
  Theme   = saved.Theme   or Veloalib.Save.Theme,
}
...
local function VerifyTheme(Theme)
	for name,_ in pairs(Veloalib.Themes) do
		if name == Theme then
			return true
		end
	end
...
local Theme = Veloalib.Themes[Veloalib.Save.Theme]

local function AddEle(Name, Func)
	Veloalib.Elements[Name] = Func
end

local function Make(Ele, Instance, props, ...)
	local Element = Veloalib.Elements[Ele](Instance, props, ...)
	return Element
end
...
-- /////////// --
function Veloalib:GetIcon(index)
...
function Veloalib:SetTheme(NewTheme)
	if not VerifyTheme(NewTheme) then return end

	Veloalib.Save.Theme = NewTheme
	SaveJson("sp library v2.json", Veloalib.Save)
	Theme = Veloalib.Themes[NewTheme]

	Connection:FireConnection("ThemeChanged", NewTheme)
	table.foreach(Veloalib.Instances, function(_,Val)
...
function Veloalib:SetScale(NewScale)
...
function Veloalib:MakeWindow(Configs)
...
function Veloalib:MakeWindow(Configs)
    ...
    Settings.RainbowMainFrameDefault = Configs.RainbowMainFrameDefault or false
    Settings.RainbowTitleDefault = Configs.RainbowTitleDefault or false
    Settings.RainbowSubTitleDefault = Configs.RainbowSubTitleDefault or false
   ...
   local ToggleIcon = tostring(Configs.ToggleIcon or "rbxassetid://83114982417764")
	...
	local UISizeX, UISizeY = unpack(Veloalib.Save.UISize)
	...
	local MainScroll = InsertTheme(Create("ScrollingFrame", Components, {
		Size = UDim2.new(0, Veloalib.Save.TabSize, 1, -TopBar.Size.Y.Offset),
...
if isPC then
    ...
    if Veloalib.Flags["sidebarHover"] and Veloalib.Flags["sidebarHover"].Value then
        enableSidebarHover()
    end
end
...
ConnectSave(ControlSize1, function()
    if not Minimized then
        Veloalib.Save.UISize = {MainFrame.Size.X.Offset, MainFrame.Size.Y.Offset}
        SaveJson("sp library v2 .json", Veloalib.Save)
    end
end)

ConnectSave(ControlSize2, function()
    Veloalib.Save.TabSize = MainScroll.Size.X.Offset
    SaveJson("sp library v2.json", Veloalib.Save)
end)
...
CloseButton.Activated:Connect(function()
    ...
    Veloalib:ClearAllBinds()

if shouldClearToggles then
    Veloalib:ClearAllToggles()
end
...
T.MouseButton1Click:Connect(function()
	...
	function Window:SelectTab(TabSelect)
		if type(TabSelect) == "number" then
			Veloalib.Tabs[TabSelect].func:Enable()
		else
			for _,Tab in pairs(Veloalib.Tabs) do
 ...
 local SettingTab = InsertTheme(Create("ScrollingFrame", {
    ...
    Create("UIListLayout", {
		...
		local function Tabs()
			...
			table.foreach(Veloalib.Tabs, function(_,Tab)
        ...
        FirstTab = true
		table.insert(Veloalib.Tabs, {TabInfo = {Name = TName, Icon = TIcon}, func = Tab, Cont = Container})
		Tab.Cont = Container
...
SettingButton.MouseButton1Click:Connect(function()
    SettingTabHandler:Enable()
		...
		function Tab:AddSection(Configs)
			...
			local Section = {}
			table.insert(Veloalib.Options, {type = "Section", Name = SectionName, func = Section})
...
function Veloalib:MakeNotification(Configs)
		...
		if state.Binding then
			...
			if newKey then
		       ...
		       if Flag then
				...
				if Flag then Veloalib.Flags[Flag] = state end
...
function Veloalib:ClearAllBinds()
...
function Veloalib:ClearAllToggles()
...
function Colorpicker:Callback(fn)
    ...
    return Colorpicker
		...
		function Tab:AddDiscordInvite(Configs)
			...
			local Desc = Configs.Desc or Configs.Description or "Join our Discord server!"
			local Logo = Configs[2] or Configs.Logo or ""
			local Invite = Configs[3] or Configs.Invite or "https://discord.gg/your-invite-link"
